...
class VaccineTrackerDataSeeder extends Seeder
{
    /**
    ...
    {
        $this->command->info('Starting Vaccine Tracker Data Seeding...');
        ...
        $this->command->info("Found {$patients->count()} patients. Adding comprehensive data...");
        ...
        $this->command->info("Vaccine Tracker Data Seeding Complete!");
        $this->command->info("- Newborn Immunization records added: {$newbornCount}");
        $this->command->info("- Nutrition 12 Months records added: {$nutritionCount}");
        $this->command->info("- Outcomes records added: {$outcomesCount}");
    }
    
    private function createNewbornData($patient)
    {
        $birthDate = Carbon::parse($patient->birth_date);
        $registrationDate = Carbon::parse($patient->registration_date);
